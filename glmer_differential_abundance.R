...
DimPlot(poms_seurat, group.by = "basic_labels")



table(poms$basic_labels, poms$patient_nice)
colData(poms)$cell_id <- paste0("Cell_", seq_len(ncol(poms)))
...
#data extraction ========================================================
#pulling out complete df of interactions for use in machine learning
fet_interactions <- subsetCommunication(fet.chat, slot.name = "net")
write.csv(fet_interactions, file = "../")